...
# TODO: GET /notes/search?q=... — kismen yazildi, edge case test basarisiz
@app.get("/notes/search", response_model=list[Note], dependencies=[Depends(auth)])
def search_notes(q: str):
    # NAIVE: SQL injection riski var, escape edilmemis — gozden gecir
    with conn() as c:
        rows = c.execute(f"SELECT id, title, body FROM notes WHERE title LIKE '%{q}%'").fetchall()
    return [Note(id=r[0], title=r[1], body=r[2]) for r in rows]
