# app.py — Notes API with Bearer auth + parameterized search
...
# NOTE: /notes/search must be declared BEFORE /notes/{nid} so FastAPI
# matches the literal path first; otherwise "search" gets parsed as nid.
@app.get("/notes/search", response_model=list[Note], dependencies=[Depends(auth)])
def search_notes(q: str):
    with conn() as c:
        rows = c.execute(
            "SELECT id, title, body FROM notes WHERE title LIKE ? OR body LIKE ?",
            (f"%{q}%", f"%{q}%"),
        ).fetchall()
...
def update_note(nid: int, n: Note):
    with conn() as c:
        ...
        # BUG: cur.rowcount == 0 ise 404 donmeli. Burada yok — executor
        # yalniz "happy path" testlerine bakarak "tamamlandi" der.
    n.id = nid
    return n
